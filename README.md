2 algorithms for comparing strings. Both return Array[number of differences, % of similarity as a float 0.0-1.0]
Additonal hybrid function that uses either algorithm as apropriate and a function for formatting strings for common users errors. Like alphabetizing them to account for jumbled spelling and truncation to avoid excessively long word checking.
An example is included, with a scrollable ordered lists of comparisons for a quick visual example of how the script works and an example of implmentation and performance.
